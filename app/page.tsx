...
export default function Home() {
  ...
  return (
    <main className="mollee-site">
      ...
      {checkoutOpen && <div className="overlay" role="dialog" aria-modal="true" aria-label="Checkout"><button className="overlay__backdrop" onClick={() => !checkoutLoading && setCheckoutOpen(false)} aria-label="Close checkout" /><div className="checkout-modal"><button className="close-button" onClick={() => setCheckoutOpen(false)} disabled={checkoutLoading}>×</button><div className="checkout-head"><span className="category-subtitle"><b>paystack</b> checkout</span><h2>Delivery details</h2><p>Choose free pickup or a delivery zone, then pay securely with Paystack.</p></div><form className="checkout-form" onSubmit={submitCheckout}><div className="field-grid"><label>First name<input name="firstName" required /></label><label>Last name<input name="lastName" required /></label></div><label>Email<input name="email" required type="email" /></label><label>Phone number<input name="phone" required type="tel" placeholder="+234" /></label><div className="shipping-choice"><div className="shipping-choice__head"><h3>Select shipping</h3><span>{selectedShipping.price === 0 ? "Free" : money(selectedShipping.price)}</span></div>{shippingOptions.map((option) => <label className={`shipping-option ${shippingOptionId === option.id ? "active" : ""}`} key={option.id}><input type="radio" name="shippingOptionId" value={option.id} checked={shippingOptionId === option.id} onChange={() => setShippingOptionId(option.id)} /><span><b>{option.title}</b><small>{option.description}</small></span><strong>{option.price === 0 ? "Free" : money(option.price)}</strong></label>)}<p>Production for T-shirts usually takes 1–2 working days. Delivery is usually the next day after production inside Abuja and 4–6 working days outside Abuja.</p></div><label>Country<select name="country" required defaultValue="Nigeria"><option>Nigeria</option><option>Ghana</option><option>United Kingdom</option><option>United States</option><option>Canada</option><option>Other international</option></select></label><label>Delivery address / pickup note<input name="address" required={shippingOptionId !== "pickup-kubwa"} placeholder={shippingOptionId === "pickup-kubwa" ? "Pickup at No 8 Emmanuel Close, Arab Road, Kubwa" : "Street address"} /></label><div className="field-grid"><label>City<input name="city" required={shippingOptionId !== "pickup-kubwa"} placeholder={shippingOptionId === "pickup-kubwa" ? "Kubwa" : ""} /></label><label>State / region<input name="state" required={shippingOptionId !== "pickup-kubwa"} placeholder={shippingOptionId === "pickup-kubwa" ? "Abuja" : ""} /></label></div><div className="checkout-total"><span>Subtotal</span><strong>{money(subtotal)}</strong></div><div className="checkout-total"><span>{selectedShipping.title}</span><strong>{delivery === 0 ? "Free" : money(delivery)}</strong></div><div className="checkout-total checkout-total--grand"><span>Order total</span><strong>{money(subtotal + delivery)}</strong></div><label className="terms"><input type="checkbox" required /> I agree to the store terms, shipping and return policy.</label>{checkoutError && <p className="checkout-error" role="alert">{checkoutError}</p>}<button className="mol-button" type="submit" disabled={checkoutLoading || cart.length === 0}><span>{checkoutLoading ? "Connecting to Paystack…" : "Pay securely with Paystack"}</span></button><small>Your order is only confirmed after Paystack verifies the payment.</small></form></div></div>}
